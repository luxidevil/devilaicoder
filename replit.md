...
- **Wave 9 — Semantic codebase search (pgvector + Gemini embeddings)**:
  - `artifacts/api-server/src/lib/embeddings.ts`: self-bootstrapping pgvector schema (CREATE EXTENSION + `file_embeddings` table with HNSW cosine index, ivfflat fallback), Gemini `text-embedding-004` (768d, batched 32) via `batchEmbedContents`, walker that skips node_modules/.git/build/binaries/lockfiles + capped at 256KB/file, 200-line chunks with 30-line overlap, sha-16 incremental cache, GC of removed chunks.
  - Two new agent tools wired in `agent.ts`: **`index_codebase`** (full or path_prefix scoped, incremental by default) and **`semantic_search`** (cosine via `<=>`, top-k with line ranges + 600-char preview).
  - System prompt updated to instruct semantic_search BEFORE grep for conceptual queries.
  - Cleanup: removed dead/duplicate `web_search` handler from Wave 1 that was shadowing the Wave 8 multi-source aggregator.
